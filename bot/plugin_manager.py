...
# Aggiungiamo le funzioni per gestire i Tool Use di Claude
    def register_claude_tool(self, name: str, description: str, input_schema: Dict[str, Any]) -> bool:
        """
        Registra un nuovo tool per l'API di Claude.
        
        Args:
            name: Nome del tool
            description: Descrizione dettagliata del tool
            input_schema: Schema JSON per l'input del tool
            
        Returns:
            bool: True se la registrazione è riuscita, False altrimenti
        """
        try:
            # Verifica che il nome sia valido (regex ^[a-zA-Z0-9_-]{1,64}$)
            import re
            if not re.match(r'^[a-zA-Z0-9_-]{1,64}
, name):
                logger.error(f"Il nome del tool '{name}' non è valido. Deve corrispondere al regex ^[a-zA-Z0-9_-]{{1,64}}$.")
                return False
            
            # Crea la definizione del tool
            tool_definition = {
                "name": name,
            ...
            # Registra il tool
            self.claude_tools[name] = tool_definition
            logger.info(f"Tool '{name}' per Claude registrato con successo.")
            return True
            
        except Exception as e:
            logger.error(f"Errore durante la registrazione del tool '{name}' per Claude: {str(e)}")
            return False
    
    def unregister_claude_tool(self, name: str) -> bool:
        """
        Cancella la registrazione di un tool per l'API di Claude.
        
        Args:
            name: Nome del tool da cancellare
            
        Returns:
            bool: True se la cancellazione è riuscita, False altrimenti
        """
        if name in self.claude_tools:
            del self.claude_tools[name]
            logger.info(f"Tool '{name}' per Claude rimosso con successo.")
            return True
        else:
            logger.warning(f"Tool '{name}' non trovato.")
            return False
    
    def get_claude_tools(self) -> List[Dict[str, Any]]:
        """
        Ottiene la lista dei tool registrati per l'API di Claude.
        
        Returns:
            List[Dict[str, Any]]: Lista delle definizioni dei tool
        """
        return list(self.claude_tools.values())
    
    def configure_tool_use(self, disable_parallel_tool_use: bool = None, tool_choice: str = None) -> None:
        """
        Configura le opzioni di Tool Use per Claude.
        
        Args:
            disable_parallel_tool_use: Se True, disabilita l'uso di tool in parallelo
            tool_choice: Tipo di scelta del tool ("auto", "any", "tool", "none")
        """
        if disable_parallel_tool_use is not None:
            self.tool_use_config["disable_parallel_tool_use"] = disable_parallel_tool_use
        
        if tool_choice is not None:
            if tool_choice not in ["auto", "any", "tool", "none"]:
                logger.warning(f"Valore non valido per tool_choice: '{tool_choice}'. Deve essere uno tra 'auto', 'any', 'tool', 'none'.")
            else:
                self.tool_use_config["tool_choice"] = tool_choice
    
    def handle_tool_use(self, tool_use_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Gestisce una richiesta di Tool Use da Claude.
        
        Args:
            tool_use_data: Dati della richiesta di Tool Use
            
        Returns:
            Dict[str, Any]: Risultato dell'esecuzione del tool
        """
        try:
            # Estrai le informazioni dal tool_use
            tool_use_id = tool_use_data.get("id")
            tool_name = tool_use_data.get("name")
            tool_input = tool_use_data.get("input", {})
            
            logger.info(f"Richiesta di Tool Use ricevuta: tool_id={tool_use_id}, tool_name={tool_name}")
            
            # Verifica se il tool è un plugin registrato
            if tool_name in self.plugins:
                # Esegui l'azione sul plugin
                result = self.execute_plugin_action(tool_name, "execute", tool_input)
                ...
                return {
                    "type": "tool_result",
                    "tool_use_id": tool_use_id,
                    "content": f"Tool '{tool_name}' non trovato.",
                    "is_error": True
                }
                
        except Exception as e:
            logger.error(f"Errore durante l'esecuzione del tool: {str(e)}")
            
            # Restituisci un errore
            return {
                "type": "tool_result",
                "tool_use_id": tool_use_id if 'tool_use_id' in locals() else "unknown",
                "content": f"Errore durante l'esecuzione del tool: {str(e)}",
                "is_error": True
            }
    
    def prepare_claude_api_parameters(self) -> Dict[str, Any]:
        """
        Prepara i parametri per l'API di Claude relativi ai tool.
        
        Returns:
            Dict[str, Any]: Parametri per l'API di Claude
        """
        result = {}
        
        # Aggiungi i tool se presenti
        if self.claude_tools:
            result["tools"] = list(self.claude_tools.values())
        
        # Aggiungi la configurazione dei tool se necessario
        if self.tool_use_config["disable_parallel_tool_use"] or self.tool_use_config["tool_choice"] != "auto":
            result["tool_choice"] = {
                "type": self.tool_use_config["tool_choice"]
            }
            
            # Se il tipo è "tool", dovrebbe esserci anche il nome del tool
            if self.tool_use_config["tool_choice"] == "tool" and len(self.claude_tools) > 0:
                # Usa il primo tool disponibile (questo dovrebbe essere configurato separatamente)
                result["tool_choice"]["name"] = next(iter(self.claude_tools.keys()))
            
            # Aggiungi disable_parallel_tool_use se True
            if self.tool_use_config["disable_parallel_tool_use"]:
...
if __name__ == "__main__":
    ...
    async def run_test():
        try:
            ...
            # Test di registrazione di tool per Claude
            print("\nTest di registrazione tool per Claude...")
            
            # Registra un tool di esempio
            weather_tool = plugin_manager.register_claude_tool(
                name="get_weather",
                description="Get the current weather in a given location. Returns temperature, conditions, and forecast.",
                input_schema={
                    "type": "object",
                    "properties": {
                        "location": {
            ...
            print(f"Tool registrato: {weather_tool}")
            
            # Ottieni e mostra i tool registrati
            claude_tools = plugin_manager.get_claude_tools()
            print(f"Tool per Claude registrati: {len(claude_tools)}")
            for tool in claude_tools:
                print(f"- {tool['name']}: {tool['description'][:50]}...")
            
            # Prepara i parametri per l'API di Claude
            api_params = plugin_manager.prepare_claude_api_parameters()
            print("\nParametri per l'API di Claude:")
            print(f"- Tools: {len(api_params.get('tools', []))}")
            if "tool_choice" in api_params:
                print(f"- Tool choice: {api_params['tool_choice']}")
